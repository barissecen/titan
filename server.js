...
const {ActivityTypes,
  ...
  io.on('connection', function(socket){
    socket.on('new-msg', async(msg) => {
      ...
      const diMes = await sessionClient2.detectIntent(msg.msg,'quickstart-session-id');
     ...
     if (outMes=="Sorry, I'm not trained to help with that. Please provide additional details and I will do my best to provide assistance.")
      {
           //request
        const Clever = new Cleverbot('API_KEY');
 
        Clever.request(msg.msg).then(function(response) {
        //console.log(response.output);
        io.emit('send-msg-response', response.output);

        //write to the mongoDB
        conver = {"user_message":msg.msg, "robot_message":response.output, "intent":diMes.intent.displayName, "intent_confidence":diMes.intentDetectionConfidence, "sentiment_score":diMes.sentimentAnalysisResult.queryTextSentiment.score};
    ...
    io.emit('send-msg-response', outMes);
    //write to the mongoDB
    conver = {"user_message":msg.msg, "robot_message":outMes, "intent":diMes.intent.displayName, "intent_confidence":diMes.intentDetectionConfidence, "sentiment_score":diMes.sentimentAnalysisResult.queryTextSentiment.score};
    db.conversation.save(conver, function(err, conver){
          //console.log(conver);
    });  
   }
